...
export default function Navigation({ activeSection, isScrolled, onSectionClick }: NavigationProps) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[1000] py-3 sm:py-3.5 md:py-4 transition-all duration-400 ${
      isScrolled 
        ? 'professional-card shadow-premium bg-white/98 dark:bg-gray-800/98 backdrop-blur-xl border-b border-border-default/50 dark:border-gray-700/50' 
        : 'bg-transparent'
    }`}>
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 md:px-8 flex justify-between items-center">
        ...
        >
          <span className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold text-text-primary dark:text-gray-50 transition-all duration-300 group-hover:text-accent-primary group-active:text-accent-primary tracking-tight">
            Portfolio
          </span>
        </div>
          ...
          <ul className="flex gap-4 lg:gap-6 list-none m-0 p-0">
            {navItems.map((item) => (
              <li key={item.id}>
                <a
                  href={`#${item.id}`}
                  className={`text-sm lg:text-base font-semibold text-text-secondary dark:text-gray-300 relative transition-all duration-300 no-underline py-2 px-1 touch-manipulation tracking-tight ${
                ...
                >
                  ...
                  {activeSection === item.id && (
                    <span className="absolute bottom-0 left-0 w-full h-1 bg-accent-primary rounded-full shadow-accent"></span>
                  )}
                </a>
              </li>
            ))}
